...
const { getVariantTitle, formatDate } = useVariantsStore();

const getPairProperty = (pair: any) => {
  if (pair.properties && pair.properties.length > 0) return pair.properties[0];
  if (pair.characteristics && pair.characteristics.length > 0)
    return pair.characteristics[0];
  return '—';
};

const getMatchingAnswer = (task2: any) => {
  return task2.pairs.map((_: any, idx: number) => idx + 1).join('');
};
</script>
<template>
  <div class="w-full max-w-[956px]">
    <!-- Header -->
    <div
      class="text-[#333333] w-full bg-white rounded-[10px] flex justify-center items-center p-5 mt-8 mb-3"
    >
      <h1
        class="w-full font-normal text-2xl leading-7 align-center text-center"
      >
          ...
          >
            ...
            <div class="flex justify-end">
              <p class="text-base font-semibold text-gray-600">
                {{ weeklyVariant.work.author }} — «{{
                  weeklyVariant.work.title
                }}»
              </p>
            </div>
          </div>
