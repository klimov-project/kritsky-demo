...
const isCommonTaskKey = (value: string | undefined): value is CommonTaskKey =>
  Boolean(value && COMMON_TASK_SET.has(value));
const isExcerptTaskKey = (value: string | undefined): value is ExcerptTaskKey =>
  Boolean(value && EXCERPT_TASK_SET.has(value));
const isPoemTaskKey = (value: string | undefined): value is PoemTaskKey =>
  Boolean(value && POEM_TASK_SET.has(value));
...
const normalizeExcerptsOrder = (items: Excerpt[]): Excerpt[] => {
  return sortByExcerptOrder(items).map((item, index) => ({
    ...item,
    order: index + 1,
  }));
};

const insertExcerptAtOrder = (
  items: Excerpt[],
  nextExcerpt: Excerpt,
  desiredOrder: number,
): Excerpt[] => {
  const normalized = normalizeExcerptsOrder(
    items.filter((item) => item.id !== nextExcerpt.id),
  );
  const normalizedOrder = Number.isFinite(desiredOrder)
    ? Math.max(1, Math.floor(desiredOrder))
    : normalized.length + 1;
...
const moveExcerptToOrder = (
  ...
  return insertExcerptAtOrder(
    items.filter((item) => item.id !== excerptId),
    target,
    desiredOrder,
  );
};
...
const createEmptyBlock3Question = (
  key: Block3Key,
): Block3Question | Block3QuestionMultiAuthor => {
  if (key === 'task11_4') {
    return {
      id: createId('b3-11-4'),
...
const isItemActive = (item: ActivatableItem | null | undefined): boolean =>
  item?.isActive !== false;
const countActiveItems = (items: readonly ActivatableItem[]): number =>
  items.filter((item) => isItemActive(item)).length;
const buildMetaParts = (
  items: Array<string | null | undefined | false>,
): string[] => items.filter(Boolean) as string[];
...
function CompactQuestionRow({
  ...
  return (
    <div
      className={`relative rounded-lg border border-[#221E20]/10 bg-[#FCFCFA] px-3 py-2 ${
        href
          ? 'hover:border-[#221E20]/25 hover:bg-[#F8F7F4] transition-colors'
          : ''
      }`}
    >
      <div className="flex items-start gap-3">
        {href ? (
          ...
          >
            <div className="flex min-w-[48px] items-center gap-2 pt-0.5">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wide opacity-45">
                #{index + 1}
              </span>
            </div>
            <div className="min-w-0 space-y-1">
              <p className="truncate text-sm font-medium text-[#221E20]">
                {title}
              </p>
        ...
        <div className="shrink-0">
          <Button
            variant="outlined"
            className="h-8 px-2 text-red-600 border-red-100"
            onClick={onDelete}
          >
...
function RichTextField({
  ...
  const buttonClassName =
    'h-[30px] min-w-[40px] rounded border border-[#221E20]/15 bg-white px-2 text-xs hover:border-[#221E20]/40';
...
const arrayToLines = (value: string[] | undefined): string =>
  (value || []).join('\n');

const stripHtmlTags = (value: string): string =>
  normalizeNbsp(value).replace(/<[^>]*>/g, ' ');
...
function ShortQuestionsEditor({
  ...
  const isDetailPage =
    Number.isInteger(selectedIndex) && selectedIndex !== undefined;
  const currentItem = isDetailPage ? items[selectedIndex!] : null;

  const updateItem = (
    index: number,
    updater: (item: ShortQuestion) => ShortQuestion,
  ) => {
    onChange(
      ...
      <section className="rounded-xl border border-[#221E20]/10 bg-white p-4 space-y-3">
        ...
        <div className="space-y-2">
          {items.map((item, index) => {
            const itemHref = baseHref
              ? buildNestedMaterialsHref(baseHref, String(index))
              : '';
            return (
              <CompactQuestionRow
                key={`${item.id}-${index}`}
          ...
          {!items.length ? (
            <p className="text-sm opacity-60">Пока нет записей.</p>
          ) : null}
        </div>
        ...
        <div className="pt-2 flex justify-end">
          <Button
            variant="outlined"
            onClick={() => onChange([...items, createShortQuestion(addPrefix)])}
          >
            Добавить
          </Button>
        </div>
      ...
      <div className="rounded-lg border border-[#221E20]/10 p-3 space-y-3">
        <div className="flex justify-end">
          <Button
            ...
            onClick={() =>
              onChange(
                items.filter((_, itemIndex) => itemIndex !== selectedIndex),
              )
            }
          >
            <IoTrashOutline size={14} />
        ...
        <div className={DETAIL_META_GRID_CLASS}>
            ...
            <>
              <div className={DETAIL_FULL_WIDTH_CLASS}>
                <RichTextField
                  ...
                  onChange={(value) =>
                    updateItem(selectedIndex!, (entry) => ({
                      ...entry,
                      text: value,
                    }))
                  }
          ...
          <TextField
            ...
            onChange={(value) =>
              updateItem(selectedIndex!, (entry) => ({ ...entry, id: value }))
            }
          />
          <StatusField
          ...
          <TextField
            ...
            onChange={(value) =>
              updateItem(selectedIndex!, (entry) => ({ ...entry, tags: value }))
            }
          />
        </div>
...
function TwoGapQuestionsEditor({
  ...
  const sanitizeItem = (item: TwoGapQuestion): TwoGapQuestion =>
    singleEntryMode ? { ...item, part2: '', answer2: '', termId2: '' } : item;

  useEffect(() => {
    if (!singleEntryMode) return;
    const hasSplitFields = items.some((item) =>
      Boolean(item.part2 || item.answer2 || item.termId2),
    );
    if (!hasSplitFields) return;
    onChange(
      items.map((item) => ({ ...item, part2: '', answer2: '', termId2: '' })),
    );
  }, [items, onChange, singleEntryMode]);

  const updateItem = (
    index: number,
    updater: (item: TwoGapQuestion) => TwoGapQuestion,
  ) => {
    onChange(
      items.map((item, itemIndex) =>
        itemIndex === index ? sanitizeItem(updater(item)) : item,
      ),
    );
  };
            ...
            return (
              <CompactQuestionRow
                ...
                meta={buildMetaParts([
                  ...
                  !singleEntryMode && item.termId2
                    ? `Термин 2: ${item.termId2}`
                    : null,
                  item.tags ? `Теги: ${item.tags}` : null,
        ...
        <div className="pt-2 flex justify-end">
          <Button
            ...
            onClick={() =>
              onChange([
                ...items,
                sanitizeItem(createTwoGapQuestion(addPrefix)),
              ])
            }
          >
            Добавить
        ...
        <div className={DETAIL_META_GRID_CLASS}>
          ...
          {!singleEntryMode ? (
            <>
              ...
              <TextField
                compact
                label="Идентификатор термина (часть 2)"
                value={currentItem.termId2 || ''}
                onChange={(value) =>
                  updateItem(selectedIndex!, (entry) => ({
                    ...entry,
                    termId2: value,
...
function EssayQuestionsEditor({
  ...
  const updateItem = (
    index: number,
    updater: (item: EssayQuestion) => EssayQuestion,
  ) => {
    onChange(
      items.map((item, itemIndex) =>
        itemIndex === index ? updater(item) : item,
    ...
    return (
      <section className="rounded-xl border border-[#221E20]/10 bg-white p-4 space-y-3">
            ...
            return (
              <CompactQuestionRow
                ...
                meta={buildMetaParts([
                  `ID: ${item.id || '—'}`,
                  item.termId ? `Термин: ${item.termId}` : null,
                  showAuthorId && item.authorId
                    ? `Автор: ${item.authorId}`
                    : null,
                  showThemePairIds && item.theme1Id
                    ? `Тема 1: ${item.theme1Id}`
                    : null,
                  showThemePairIds && item.theme2Id
                    ? `Тема 2: ${item.theme2Id}`
                    : null,
                  item.themeInternalId
                    ? `Внутренняя тема: ${item.themeInternalId}`
                    : null,
                  showSimilarityId && item.similarityId
                    ? `Сопоставление: ${item.similarityId}`
                    : null,
                  showPublicId && item.publicId
                    ? `Публичный ID: ${item.publicId}`
                    : null,
                  item.tags ? `Теги: ${item.tags}` : null,
                ])}
                isActive={item.isActive}
        ...
        <div className="pt-2 flex justify-end">
          <Button
            variant="outlined"
            onClick={() => onChange([...items, createEssayQuestion(addPrefix)])}
          >
            Добавить
          </Button>
        </div>
      ...
      <div className="rounded-lg border border-[#221E20]/10 p-3 space-y-3">
        ...
        <div className={DETAIL_META_GRID_CLASS}>
          ...
          {showThemePairIds ? (
            <TextField
              compact
              label="Идентификатор темы 1"
              value={currentItem.theme1Id || ''}
              onChange={(value) =>
                updateItem(selectedIndex!, (entry) => ({
                  ...entry,
                  theme1Id: value,
          ...
          {showThemePairIds ? (
            <TextField
              compact
              label="Идентификатор темы 2"
              value={currentItem.theme2Id || ''}
              onChange={(value) =>
                updateItem(selectedIndex!, (entry) => ({
                  ...entry,
                  theme2Id: value,
          ...
          {showSimilarityId ? (
            <TextField
              compact
              label="Идентификатор типа сопоставления"
              value={currentItem.similarityId || ''}
              onChange={(value) =>
                updateItem(selectedIndex!, (entry) => ({
                  ...entry,
                  similarityId: value,
          ...
          <TextField
            ...
            value={currentItem.themeInternalId || ''}
            onChange={(value) =>
              updateItem(selectedIndex!, (entry) => ({
                ...entry,
                themeInternalId: value,
              }))
            }
          />
          {showPublicId ? (
...
function MatchingQuestionsEditor({
  ...
  const updateItem = (
    index: number,
    updater: (item: MatchingQuestion) => MatchingQuestion,
  ) => {
    onChange(
      items.map((item, itemIndex) =>
        itemIndex === index ? updater(item) : item,
  ...
  const getCurrentPropertyCategory = (
    item: MatchingQuestion,
  ): MatchingQuestion['pairPropertyType'] => item.pairPropertyType || 'phrases';
  const getPairPropertyValues = (
    pair: MatchPair,
    category: MatchingQuestion['pairPropertyType'],
  ): string[] => {
    if (category === 'characteristics') {
      return pair.characteristics?.length
      ...
      <section className="rounded-xl border border-[#221E20]/10 bg-white p-4 space-y-3">
            ...
            return (
              <CompactQuestionRow
                ...
                meta={buildMetaParts([
                  ...
                  item.tags ? `Теги: ${item.tags}` : null,
                  item.characterCount
                    ? `Персонажей: ${item.characterCount}`
                    : null,
        ...
        <div className="pt-2 flex justify-end">
          <Button
            variant="outlined"
            onClick={() =>
              onChange([...items, createMatchingQuestion(addPrefix)])
            }
          >
            Добавить
          </Button>
      ...
      <div className="rounded-lg border border-[#221E20]/10 p-3 space-y-3">
              ...
              onChange={(event) =>
                updateItem(selectedIndex!, (entry) => {
                  const nextCategory = event.target
                    .value as MatchingQuestion['pairPropertyType'];
                  return {
                    ...entry,
                    pairPropertyType: nextCategory,
                    pairs: entry.pairs.map((pair) =>
                      setPairPropertyValues(
                        pair,
                        nextCategory,
                        getPairPropertyValues(pair, nextCategory),
                      ),
                    ),
                  };
                })
        ...
        <div className="rounded-lg border border-[#221E20]/10 p-3 space-y-3">
          ...
          {currentItem.pairs.map((pair) => (
            <div
              key={pair.id}
              className="rounded-lg border border-[#221E20]/10 p-3 space-y-2"
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <Button
                  ...
                  onClick={() =>
                    updateItem(selectedIndex!, (entry) => ({
                      ...entry,
                      pairs: entry.pairs.filter((p) => p.id !== pair.id),
                    }))
                  }
                >
                  <IoTrashOutline size={14} />
              ...
              <div className={DETAIL_META_GRID_CLASS}>
                ...
                <TextField
                  ...
                  onChange={(value) =>
                    updateItem(selectedIndex!, (entry) => ({
                      ...entry,
                      pairs: entry.pairs.map((p) =>
                        p.id === pair.id ? { ...p, character: value } : p,
                      ),
                    }))
                  }
                />
                ...
                <div className={DETAIL_FULL_WIDTH_CLASS}>
                  <RichStringListField
                    label={
                      getCurrentPropertyCategory(currentItem) ===
                      'characteristics'
                        ? 'Характеристики'
                        : 'Фразы'
                    }
                    value={getPairPropertyValues(
                      pair,
                      getCurrentPropertyCategory(currentItem),
                    )}
                    onChange={(value) =>
                      updateItem(selectedIndex!, (entry) => ({
                        ...
                        pairs: entry.pairs.map((p) =>
                          p.id === pair.id
                            ? setPairPropertyValues(
                                p,
                                getCurrentPropertyCategory(entry),
                                value,
                              )
                            : p,
                        ),
                  ...
                  />
                  {getPairPropertyValues(
                    pair,
                    getCurrentPropertyCategory(currentItem),
                  ).length > 0 && (
                    <div className="mt-2 space-y-1">
                      <FieldLabel>
                        Идентификаторы свойств (для исключения)
                      </FieldLabel>
          ...
          <div className="pt-2 flex justify-end">
            <Button
              ...
              onClick={() =>
                updateItem(selectedIndex!, (entry) => ({
                  ...entry,
                  pairs: [...entry.pairs, createMatchPair()],
                }))
              }
            >
              Добавить пару
...
function CharactersEditor({
  ...
  const updateItem = (
    index: number,
    updater: (item: Character) => Character,
  ) => {
    onChange(
      items.map((item, itemIndex) =>
        itemIndex === index ? updater(item) : item,
      ...
      <section className="rounded-xl border border-[#221E20]/10 bg-white p-4 space-y-3">
        ...
        <div className="space-y-2">
          {items.map((item, index) => (
            <div
              key={`${item.id}-${index}`}
              className={`relative rounded-lg border border-[#221E20]/10 px-3 py-2 flex items-center justify-between gap-3 ${
                baseHref
                  ? 'hover:border-[#221E20]/30 hover:bg-black/[0.02] transition-colors'
                  : ''
              }`}
            >
              {baseHref ? (
                <Link
                  href={buildNestedMaterialsHref(baseHref, String(index))}
        ...
        <div className="pt-2 flex justify-end">
          <Button
            variant="outlined"
            onClick={() => onChange([...items, createCharacter()])}
          >
            Добавить персонажа
          </Button>
        </div>
      ...
      <div className="rounded-lg border border-[#221E20]/10 p-3 space-y-3">
        ...
        <div className={DETAIL_META_GRID_CLASS}>
          ...
          <TextField
            ...
            onChange={(value) =>
              updateItem(selectedIndex!, (entry) => ({ ...entry, name: value }))
            }
          />
          <TextField
            compact
            ...
            onChange={(value) =>
              updateItem(selectedIndex!, (entry) => ({ ...entry, tag: value }))
            }
          />
          <div className={DETAIL_FULL_WIDTH_CLASS}>
            <StringListField
              label="Цитаты"
              value={currentItem.quotes}
              onChange={(value) =>
                updateItem(selectedIndex!, (entry) => ({
                  ...entry,
                  quotes: value,
...
function MultiSelectQuestionsEditor({
  ...
  const updateItem = (
    index: number,
    updater: (item: MultiSelectQuestion) => MultiSelectQuestion,
  ) => {
    onChange(
      items.map((item, itemIndex) =>
        itemIndex === index ? updater(item) : item,
    ...
    return (
      <section className="rounded-xl border border-[#221E20]/10 bg-white p-4 space-y-3">
            ...
            return (
              <CompactQuestionRow
                ...
                meta={buildMetaParts([
                  ...
                  item.tags ? `Теги: ${item.tags}` : null,
                  item.options.length
                    ? `Вариантов: ${item.options.length}`
                    : null,
                  item.options.some((option) => option.isCorrect)
                    ? `Верных: ${
                        item.options.filter((option) => option.isCorrect).length
                      }`
                    : 'Верных: 0',
                ])}
                isActive={item.isActive}
        ...
        <div className="pt-2 flex justify-end">
          <Button
            variant="outlined"
            onClick={() => onChange([...items, createMultiSelectQuestion()])}
          >
            Добавить
          </Button>
        </div>
      ...
      <div className="rounded-lg border border-[#221E20]/10 p-3 space-y-3">
        ...
        <div className="rounded-lg border border-[#221E20]/10 p-3 space-y-3">
          ...
          {currentItem.options.map((option) => (
            <div
              key={option.id}
              className="rounded-lg border border-[#221E20]/10 p-2 flex items-center gap-3 bg-white"
            >
              <Checkbox
                label="Верно"
                ...
                <TextField
                  ...
                  onChange={(value) =>
                    updateItem(selectedIndex!, (entry) => ({
                      ...entry,
                      options: entry.options.map((opt) =>
                        opt.id === option.id ? { ...opt, id: value } : opt,
                      ),
                    }))
                  }
                />
                <TextField
                  ...
                  onChange={(value) =>
                    updateItem(selectedIndex!, (entry) => ({
                      ...entry,
                      options: entry.options.map((opt) =>
                        opt.id === option.id ? { ...opt, term: value } : opt,
                      ),
                    }))
                  }
                />
                <TextField
                  ...
                  onChange={(value) =>
                    updateItem(selectedIndex!, (entry) => ({
                      ...entry,
                      options: entry.options.map((opt) =>
                        opt.id === option.id ? { ...opt, termId: value } : opt,
                      ),
                    }))
                  }
                />
          ...
          <div className="pt-2 flex justify-end">
            <Button
              ...
              onClick={() =>
                updateItem(selectedIndex!, (entry) => ({
                  ...entry,
                  options: [...entry.options, createMultiSelectOption()],
                }))
              }
            >
              Добавить вариант
          ...
          {COMMON_TASK_KEYS.map((key) => (
            ...
            >
              ...
              <p className="text-xs opacity-60 mt-1">
                Активно: {countActiveItems(value[key])} / {value[key].length}
              </p>
            </Link>
          ))}
        </div>
    ...
    <nav className="flex flex-wrap items-center gap-2 text-sm mb-6">
      {items.map((item, index) => {
        const hasMenu = Boolean(
          (item.siblings && item.siblings.length) || item.onAdd,
        );
        return (
          <React.Fragment key={`${item.href}-${item.label}-${index}`}>
            ...
            <div className="relative group flex items-center h-full">
              <Link
                href={item.href}
                className="rounded px-2 py-1 hover:bg-black/5 transition-colors"
              >
                {item.label}
              </Link>
...
export default function AdminMaterialsPages() {
  ...
  const replaceWork = useCallback(
    (workId: Id, updater: (work: Work) => Work) => {
      setWorks((prev) =>
        prev.map((work) => (work.id === workId ? updater(work) : work)),
      );
    },
    [],
  );

  const replacePoet = useCallback(
    (poetId: Id, updater: (poet: Poet) => Poet) => {
      setPoets((prev) =>
        prev.map((poet) => (poet.id === poetId ? updater(poet) : poet)),
      );
    },
    [],
  );
  ...
  const workSubSection: WorkSubSectionKey | undefined =
    ...
    rawWorkSubSection === 'characters'
      ...
      : undefined;
  const excerptId =
    section === 'works' && workSubSection === 'excerpts' ? slug[3] : undefined;

  const poetId = section === 'poets' ? slug[1] : undefined;
  const rawPoetSubSection = section === 'poets' ? slug[2] : undefined;
  const poetSubSection: PoetSubSectionKey | undefined =
    rawPoetSubSection === 'poems' ? 'poems' : undefined;
  const poemId =
    section === 'poets' && poetSubSection === 'poems' ? slug[3] : undefined;

  const block3Key =
    section === 'block3' ? (slug[1] as Block3Key | undefined) : undefined;
  const commonTaskSegment =
    section === 'works' && workSubSection === 'common-questions'
      ? slug[3]
      : undefined;
  const commonTaskKey = isCommonTaskKey(commonTaskSegment)
    ? commonTaskSegment
    : undefined;
  const commonQuestionIndex =
    section === 'works' &&
    workSubSection === 'common-questions' &&
    commonTaskKey
      ? parseIndexSegment(slug[4])
      : undefined;
  const charactersQuestionIndex =
    section === 'works' && workSubSection === 'characters'
      ? parseIndexSegment(slug[3])
      : undefined;

  const excerptNestedScope =
    section === 'works' && workSubSection === 'excerpts' ? slug[4] : undefined;
  const excerptTaskSegment =
    excerptNestedScope === 'tasks' ? slug[5] : undefined;
  const excerptTaskKey = isExcerptTaskKey(excerptTaskSegment)
    ? excerptTaskSegment
    : undefined;
  const excerptQuestionIndex =
    section === 'works' && workSubSection === 'excerpts' && excerptTaskKey
      ? parseIndexSegment(slug[6])
      : undefined;

  const poemNestedScope =
    section === 'poets' && poetSubSection === 'poems' ? slug[4] : undefined;
  const poemTaskSegment = poemNestedScope === 'tasks' ? slug[5] : undefined;
  const poemTaskKey = isPoemTaskKey(poemTaskSegment)
    ? poemTaskSegment
    : undefined;
  const poemQuestionIndex =
    section === 'poets' && poetSubSection === 'poems' && poemTaskKey
      ? parseIndexSegment(slug[6])
      : undefined;
  const block3QuestionIndex =
    section === 'block3' ? parseIndexSegment(slug[2]) : undefined;
  ...
  const currentExcerpt = useMemo(
    () =>
      currentWork && excerptId
        ? currentWork.excerpts.find((excerpt) => excerpt.id === excerptId) ||
          null
        : null,
    [currentWork, excerptId],
  );
  ...
  const currentPoem = useMemo(
    () =>
      currentPoet && poemId
        ? currentPoet.poems.find((poem) => poem.id === poemId) || null
        : null,
    [currentPoet, poemId],
  );
  ...
  const addWorkAndOpen = useCallback(async () => {
    ...
    setWorks(nextWorks);
    const saved = await persistKnowledgeBase({
      works: nextWorks,
      poets,
      block3,
    });
  ...
  const addExcerpt = useCallback(
    async (targetWorkId: string, desiredOrder?: number) => {
      const target = works.find((work) => work.id === targetWorkId);
      const baseExcerpts = target?.excerpts || [];
      const nextOrder =
        desiredOrder ??
        (baseExcerpts.length
          ? Math.max(...baseExcerpts.map((item) => item.order)) + 1
          : 1);
  ...
  const addPoemAndOpen = useCallback(
    async (targetPoetId: string) => {
      ...
      if (!saved) return;
      router.push(
        buildMaterialsHref('poets', targetPoetId, 'poems', nextPoem.id),
      );
    },
    [block3, persistKnowledgeBase, poets, router, works],
  );

  const addBlock3Question = useCallback(
    async (key: Block3Key) => {
  ...
  const breadcrumbs = useMemo<BreadcrumbItem[]>(() => {
    ...
    if (section === 'works' && workId) {
      const workSectionSiblings: BreadcrumbSibling[] = [
        {
          label: 'Карточка произведения',
          href: buildMaterialsHref('works', workId),
        },
        {
          label: 'Отрывки',
          href: buildMaterialsHref('works', workId, 'excerpts'),
        },
        {
          label: 'Общие вопросы',
          href: buildMaterialsHref('works', workId, 'common-questions'),
        },
      ];
      ...
      result.push({
        ...
        href: buildMaterialsHref('works', workId),
        siblings: works.map((work) => ({
          label: work.title || work.id,
          href: buildMaterialsHref('works', work.id),
        })),
      ...
      if (workSubSection) {
        result.push({
          ...
          addLabel:
            workSubSection === 'excerpts' ? 'Добавить отрывок' : undefined,
          onAdd:
            workSubSection === 'excerpts' && currentWork
              ? () => addExcerpt(currentWork.id)
              : undefined,
        });
      }
        ...
        if (excerptId) {
          ...
          if (excerptTaskKey) {
            ...
            if (excerptQuestionIndex !== undefined) {
              const item =
                currentExcerpt?.tasks[excerptTaskKey][excerptQuestionIndex];
              result.push({
      ...
      if (workSubSection === 'common-questions' && commonTaskKey) {
        ...
        if (commonQuestionIndex !== undefined) {
          const item =
            currentWork?.commonTasks[commonTaskKey][commonQuestionIndex];
          result.push({
      ...
      if (
        workSubSection === 'characters' &&
        charactersQuestionIndex !== undefined
      ) {
        const item = currentWork?.characters[charactersQuestionIndex];
        result.push({
          label:
            item?.name || item?.id || `Персонаж ${charactersQuestionIndex + 1}`,
          href: buildMaterialsHref(
            'works',
            workId,
            'characters',
            String(charactersQuestionIndex),
          ),
        });
      }
    }
    ...
    if (section === 'poets' && poetId) {
      result.push({
        ...
        siblings: poets.map((poet) => ({
          label: poet.name || poet.id,
          href: buildMaterialsHref('poets', poet.id),
        })),
  ...
  const renderRoot = () => {
    return (
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Link
          href={buildMaterialsHref('works')}
          className="bg-white border border-[#221E20]/10 rounded-xl p-5 hover:border-[#221E20]/30 transition-colors"
        >
          <p className="font-bold">Произведения</p>
          <p className="text-sm opacity-60 mt-1">{works.length} шт.</p>
        </Link>
        <Link
          href={buildMaterialsHref('poets')}
          className="bg-white border border-[#221E20]/10 rounded-xl p-5 hover:border-[#221E20]/30 transition-colors"
        >
        ...
        <Link
          href={buildMaterialsHref('block3')}
          className="bg-white border border-[#221E20]/10 rounded-xl p-5 hover:border-[#221E20]/30 transition-colors"
        >
          <p className="font-bold">Блок 3 (11.1–11.5)</p>
          <p className="text-sm opacity-60 mt-1">
            Всего:{' '}
            {block3.task11_1.length +
              block3.task11_2_3.length +
              block3.task11_4.length +
              block3.task11_5.length}
          </p>
        </Link>
        <Link
          href={buildMaterialsHref('settings')}
          className="bg-white border border-[#221E20]/10 rounded-xl p-5 hover:border-[#221E20]/30 transition-colors"
        >
      ...
      <section className="rounded-xl border border-[#221E20]/10 bg-white p-5 space-y-5">
        <div className="space-y-1">
          ...
          <p className="text-sm opacity-60">
            Эти тексты показываются на странице варианта в свёрнутом виде с
            кнопкой разворота.
          </p>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {works.map((work) => (
            <div
              key={work.id}
              className="relative bg-white border border-[#221E20]/10 rounded-xl p-4 hover:border-[#221E20]/30 hover:bg-black/[0.02] transition-colors"
            >
              <Link
                href={buildMaterialsHref('works', work.id)}
                className="absolute inset-0 z-10 rounded-xl"
                aria-label={`Открыть произведение ${work.title || work.id}`}
              />
              <div className="relative z-20 flex items-center justify-between gap-3 ">
                <div>
                  <p className="font-bold">{work.title || 'Без названия'}</p>
                  <p className="text-sm opacity-60">
                    {work.author || 'Без автора'} · отрывков:{' '}
                    {work.excerpts.length}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    onClick={() => {
                      if (
                        !window.confirm(
                          `Удалить произведение «${work.title || work.id}»?`,
                        )
                      )
                        return;
    ...
    if (!currentWork) {
      return (
        <NotFoundPane
          ...
          onBack={() => router.push(buildMaterialsHref('works'))}
        />
      );
    }
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-xl font-bold">
            {currentWork.title || 'Произведение'}
          </h2>
          <div className="flex items-center gap-2">
            <Button
              variant="outlined"
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          >
            ...
            <p className="text-sm opacity-60 mt-1">
              {currentWork.excerpts.length} шт.
            </p>
          </Link>
          <Link
            href={buildMaterialsHref(
          ...
          >
            ...
            <p className="text-sm opacity-60 mt-1">
              {currentWork.commonTasks.task1.length +
                currentWork.commonTasks.task2.length +
                currentWork.commonTasks.task3.length}{' '}
              шт.
            </p>
          </Link>
        </section>
        ...
        <section className="bg-white border border-[#221E20]/10 rounded-xl p-4 grid grid-cols-1 md:grid-cols-3 gap-3">
          <TextField
            ...
            onChange={(value) =>
              replaceWork(currentWork.id, (item) => ({ ...item, title: value }))
            }
          />
          <TextField
            label="Автор"
            ...
            onChange={(value) =>
              replaceWork(currentWork.id, (item) => ({
                ...item,
                author: value,
              }))
            }
          ...
          <div className={DETAIL_FULL_WIDTH_CLASS}>
            <TextField
              label="Внутренние теги"
              value={currentWork.internalTags}
              onChange={(value) =>
                replaceWork(currentWork.id, (item) => ({
                  ...item,
                  internalTags: value,
          ...
          <div className={DETAIL_FULL_WIDTH_CLASS}>
            <TextField
              label="Публичные теги"
              value={currentWork.externalTags}
              onChange={(value) =>
                replaceWork(currentWork.id, (item) => ({
                  ...item,
                  externalTags: value,
  ...
  const renderCommonQuestionsPage = (
    taskKey?: CommonTaskKey,
    selectedIndex?: number,
  ) => {
    if (!currentWork) {
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              variant="outlined"
              onClick={() =>
                router.push(buildMaterialsHref('works', currentWork.id))
              }
            >
        ...
        <CommonTasksEditor
          ...
          selectedIndex={selectedIndex}
          onChange={(value) => {
            const previousSignature = getCommonTasksCountSignature(
              currentWork.commonTasks,
            );
  ...
  const renderCharactersPage = (selectedIndex?: number) => {
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-xl font-bold">
            Банк персонажей: {currentWork.title}
          </h2>
          <Button
            variant="outlined"
            onClick={() =>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          {orderedExcerpts.map((excerpt) => (
            <div
              key={excerpt.id}
              className="group relative bg-white border border-[#221E20]/10 rounded-xl p-4 space-y-3 hover:border-[#221E20]/30 hover:bg-black/[0.02] transition-colors"
            >
              <Link
                href={buildMaterialsHref(
                  'works',
                ...
                className="absolute inset-0 z-10 rounded-xl"
                aria-label={`Открыть отрывок ${excerpt.title || excerpt.id}`}
              />
              <div className="relative z-20 flex items-center justify-between gap-3 ">
                <div>
                  <p className="flex items-center gap-1.5 font-bold">
                    <StatusChip isActive={excerpt.isActive} />
                    {excerpt.title || excerpt.id}
                  </p>
                  <p className="text-sm opacity-60">
                    Порядок: {excerpt.order} · идентификатор:{' '}
                    {excerpt.excerptId}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    onClick={() => {
                      if (
                        !window.confirm(
                          `Удалить отрывок «${excerpt.title || excerpt.id}»?`,
                        )
                      )
                        return;
                      const nextExcerpts = currentWork.excerpts.filter(
                        (entry) => entry.id !== excerpt.id,
                      );
                      const nextWorks = works.map((w) =>
                        w.id === currentWork.id
                          ? { ...w, excerpts: nextExcerpts }
                          : w,
                      );
                      setWorks(nextWorks);
                      void persistKnowledgeBase({
        ...
        <div className="pt-2 flex justify-end">
          <Button
            variant="outlined"
            className="flex items-center gap-2"
            onClick={() => addExcerpt(currentWork.id)}
          >
    ...
    const updateExcerpt = (updater: (excerpt: Excerpt) => Excerpt) => {
      replaceWork(currentWork.id, (work) => ({
        ...
        excerpts: work.excerpts.map((excerpt) =>
          excerpt.id === currentExcerpt.id ? updater(excerpt) : excerpt,
        ),
      }));
    };
    ...
    return (
      <div className="space-y-4">
            ...
            <Button
              ...
              onClick={() => {
                if (
                  ...
                  return;
                const nextExcerpts = currentWork.excerpts.filter(
                  (entry) => entry.id !== currentExcerpt.id,
                );
                const nextWorks = works.map((w) =>
                  w.id === currentWork.id
                    ? { ...w, excerpts: nextExcerpts }
                ...
                void persistKnowledgeBase({ works: nextWorks, poets, block3 });
                router.push(
                  buildMaterialsHref('works', currentWork.id, 'excerpts'),
                );
              }}
            >
              <IoTrashOutline size={16} />
        ...
        <ExcerptTasksEditor
          ...
          selectedIndex={selectedIndex}
          onChange={(value) => {
            const previousSignature = getExcerptTasksCountSignature(
              currentExcerpt.tasks,
            );
        ...
        {!taskKey ? (
          <section className="bg-white border border-[#221E20]/10 rounded-xl p-4 grid grid-cols-1 md:grid-cols-3 gap-3">
            <TextField
              label="Название"
              value={currentExcerpt.title}
              onChange={(value) =>
                updateExcerpt((item) => ({ ...item, title: value }))
              }
            />
            <TextField
              label="Глава"
              value={currentExcerpt.chapter || ''}
              onChange={(value) =>
                updateExcerpt((item) => ({ ...item, chapter: value }))
              }
            />
            <TextField
              compact
              label="Идентификатор отрывка"
              value={currentExcerpt.excerptId}
              onChange={(value) =>
                updateExcerpt((item) => ({ ...item, excerptId: value }))
              }
            />
            <TextField
              compact
              label="Тема (исключающий тег)"
              value={currentExcerpt.themeInternalId || ''}
              onChange={(value) =>
                updateExcerpt((item) => ({ ...item, themeInternalId: value }))
              }
            />
            <StatusField
              value={currentExcerpt.isActive}
              onChange={(value) =>
                updateExcerpt((item) => ({ ...item, isActive: value }))
              }
            />
            <TextField
              compact
              ...
              {currentExcerpt.textColumns === 2 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <RichTextField
                    ...
                    onChange={(value) =>
                      updateExcerpt((item) => ({ ...item, text: value }))
                    }
                    minHeight={320}
                  />
                  <RichTextField
          ...
          {poets.map((poet) => (
            ...
            >
              <Link
                href={buildMaterialsHref('poets', poet.id)}
                className="absolute inset-0 z-10 rounded-xl"
                aria-label={`Открыть автора ${poet.name || poet.id}`}
              />
              <div className="relative z-20 flex items-center justify-between gap-3 ">
                <div>
                  <p className="font-bold">{poet.name || poet.id}</p>
                  <p className="text-sm opacity-60">
                    Идентификатор автора: {poet.authorId || '—'} · стихов:{' '}
                    {poet.poems.length}
                  </p>
                </div>
                <Button
                  ...
                  onClick={() => {
                    if (
                      !window.confirm(
                        `Удалить автора «${poet.name || poet.id}»?`,
                      )
                    )
                      return;
                    const nextPoets = poets.filter(
                      (item) => item.id !== poet.id,
                    );
                    setPoets(nextPoets);
                    void persistKnowledgeBase({
                      works,
  ...
  const renderPoetDetail = () => {
    if (!currentPoet) {
      return (
        <NotFoundPane
          ...
          onBack={() => router.push(buildMaterialsHref('poets'))}
        />
      );
    }
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-xl font-bold">
            {currentPoet.name || currentPoet.id}
          </h2>
          <div className="flex items-center gap-2">
            <Button
              variant="outlined"
              ...
              onClick={() => {
                if (
                  !window.confirm(
                    `Удалить автора «${currentPoet.name || currentPoet.id}»?`,
                  )
                )
                  return;
                const nextPoets = poets.filter(
                  (item) => item.id !== currentPoet.id,
                );
                setPoets(nextPoets);
        ...
        <section className="bg-white border border-[#221E20]/10 rounded-xl p-4 grid grid-cols-1 md:grid-cols-3 gap-3">
          <TextField
            label="Имя"
            value={currentPoet.name}
            onChange={(value) =>
              replacePoet(currentPoet.id, (item) => ({ ...item, name: value }))
            }
          />
          <TextField
            compact
            label="Идентификатор автора"
            value={currentPoet.authorId}
            onChange={(value) =>
              replacePoet(currentPoet.id, (item) => ({
                ...item,
                authorId: value,
              }))
            }
      ...
      <div className="space-y-4">
          ...
          {currentPoet.poems.map((poem) => (
            ...
            >
              <Link
                ...
                className="absolute inset-0 z-10 rounded-xl"
                aria-label={`Открыть стихотворение ${poem.title || poem.id}`}
              />
              <div className="relative z-20 flex items-center justify-between gap-3 ">
                <div>
                  <p className="flex items-center gap-1.5 font-bold">
                    <StatusChip isActive={poem.isActive} />
                    {poem.title || poem.id}
                  </p>
                  <p className="text-sm opacity-60">
                    Идентификатор стихотворения: {poem.poemId || '—'}
                  </p>
                </div>
                <Button
                  variant="outlined"
                  ...
                  onClick={() => {
                    if (
                      !window.confirm(
                        `Удалить стихотворение «${poem.title || poem.id}»?`,
                      )
                    )
                      return;
                    const nextPoems = currentPoet.poems.filter(
                      (entry) => entry.id !== poem.id,
                    );
                    const nextPoets = poets.map((p) =>
                      p.id === currentPoet.id ? { ...p, poems: nextPoems } : p,
                    );
                    setPoets(nextPoets);
                    void persistKnowledgeBase({
                      works,
        ...
        <div className="pt-2 flex justify-end">
          <Button
            variant="outlined"
            className="flex items-center gap-2"
            onClick={() => addPoemAndOpen(currentPoet.id)}
          >
  ...
  const renderPoemDetail = (taskKey?: PoemTaskKey, selectedIndex?: number) => {
    if (!currentPoet || !currentPoem) {
      return (
        <NotFoundPane
          title="Стихотворение не найдено"
          onBack={() => router.push(buildMaterialsHref('poets'))}
        />
      );
    }
    ...
    const updatePoem = (updater: (poem: Poem) => Poem) => {
      replacePoet(currentPoet.id, (poet) => ({
        ...
        poems: poet.poems.map((poem) =>
          poem.id === currentPoem.id ? updater(poem) : poem,
        ),
      }));
    };
    ...
    return (
      <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() => {
                ...
                void persistKnowledgeBase({ works, poets: nextPoets, block3 });
                router.push(
                  buildMaterialsHref('poets', currentPoet.id, 'poems'),
                );
              }}
            >
              <IoTrashOutline size={16} />
        ...
        <PoemTasksEditor
          ...
          selectedIndex={selectedIndex}
          onChange={(value) => {
            const previousSignature = getPoemTasksCountSignature(
              currentPoem.tasks,
            );
            ...
            const nextPoets = poets.map((poet) => {
              ...
              return {
                ...poet,
                poems: poet.poems.map((poem) =>
                  poem.id === currentPoem.id ? { ...poem, tasks: value } : poem,
                ),
              };
            });
            setPoets(nextPoets);
        ...
        {!taskKey ? (
          <section className="bg-white border border-[#221E20]/10 rounded-xl p-4 grid grid-cols-1 md:grid-cols-3 gap-3">
            <TextField
              ...
              onChange={(value) =>
                updatePoem((item) => ({ ...item, title: value }))
              }
            />
            <TextField
              compact
              label="Идентификатор стихотворения"
              value={currentPoem.poemId}
              onChange={(value) =>
                updatePoem((item) => ({ ...item, poemId: value }))
              }
            />
            <TextField
              compact
              label="Тема (исключающий тег)"
              value={currentPoem.themeInternalId || ''}
              onChange={(value) =>
                updatePoem((item) => ({ ...item, themeInternalId: value }))
              }
            />
            <StatusField
              value={currentPoem.isActive}
              onChange={(value) =>
                updatePoem((item) => ({ ...item, isActive: value }))
              }
            />
            <label className="flex flex-col gap-1">
              <FieldLabel>Колонки текста</FieldLabel>
              ...
              {currentPoem.textColumns === 2 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <RichTextField
                    ...
                    onChange={(value) =>
                      updatePoem((item) => ({ ...item, text: value }))
                    }
                    minHeight={320}
                  />
                  <RichTextField
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {entries.map((entry) => (
            ...
            >
              ...
              <p className="text-sm opacity-60 mt-1">
                Активно: {entry.activeCount} / {entry.count}
              </p>
            </Link>
          ))}
        </div>
  ...
  const renderBlock3Group = (selectedIndex?: number) => {
    if (
      !block3Key ||
      !Object.prototype.hasOwnProperty.call(BLOCK3_LABELS, block3Key)
    ) {
      return (
        <NotFoundPane
          title="Группа блока 3 не найдена"
          onBack={() => router.push(buildMaterialsHref('block3'))}
        />
      );
    }
    ...
    const updateItem = (
      index: number,
      updater: (
        value: Block3Question | Block3QuestionMultiAuthor,
      ) => Block3Question | Block3QuestionMultiAuthor,
    ) => {
      setBlock3((prev) => ({
        ...prev,
        [block3Key]: prev[block3Key].map((item, itemIndex) =>
          itemIndex === index ? (updater(item) as never) : item,
        ),
      }));
    };
    ...
    if (!isDetailPage) {
      return (
        <section className="rounded-xl border border-[#221E20]/10 bg-white p-4 space-y-3">
          <h2 className="text-xl font-bold">
            Вопросы {BLOCK3_LABELS[block3Key]}
          </h2>

          <div className="space-y-2">
            {items.map((item, index) => {
              const itemHref = buildMaterialsHref(
                'block3',
                block3Key,
                String(index),
              );
              return (
                <CompactQuestionRow
                  ...
                  meta={buildMetaParts([
                    `ID: ${item.id || '—'}`,
                    'authorId' in item && item.authorId
                      ? `Автор: ${item.authorId}`
                      : null,
                    'authorIds' in item && item.authorIds.length
                      ? `Авторы: ${item.authorIds.join(', ')}`
                      : null,
                    item.termId ? `Термин: ${item.termId}` : null,
                    item.themeInternalId
                      ? `Тема: ${item.themeInternalId}`
                      : null,
                  ...
                  onDelete={() => {
                    ...
                    const nextBlock3 = {
                      ...block3,
                      [block3Key]: block3[block3Key].filter(
          ...
          <div className="pt-2 flex justify-end">
            <Button
              variant="outlined"
              className="flex items-center gap-2"
              onClick={() => addBlock3Question(block3Key)}
            >
      ...
      <section className="rounded-xl border border-[#221E20]/10 bg-white p-4 space-y-4">
        ...
        <div className="rounded-lg border border-[#221E20]/10 p-3 space-y-3">
          ...
          <RichTextField
            ...
            onChange={(value) =>
              updateItem(selectedIndex!, (entry) => ({ ...entry, text: value }))
            }
            minHeight={180}
          />
          ...
          <div className={DETAIL_META_GRID_CLASS}>
            ...
            {isMultiAuthor ? (
              <div className={DETAIL_FULL_WIDTH_CLASS}>
                <label className="flex flex-col gap-1">
                  <FieldLabel>
                    Идентификаторы авторов (через запятую)
                  </FieldLabel>
                  <input
  ...
  const renderContent = () => {
    ...
    if (section === 'settings') {
      ...
      return (
        <NotFoundPane
          title="Раздел оформления варианта не найден"
          onBack={() => router.push(buildMaterialsHref('settings'))}
        />
      );
    }
    ...
    if (section === 'works') {
      ...
      if (!workSubSection)
        return (
          <NotFoundPane
            title="Раздел произведений не найден"
            onBack={() => router.push(buildMaterialsHref('works'))}
          />
        );
      ...
      if (workSubSection === 'characters') {
        ...
        if (slug.length === 4) {
          if (charactersQuestionIndex === undefined)
            return (
              <NotFoundPane
                title="Раздел произведений не найден"
                onBack={() => router.push(buildMaterialsHref('works'))}
    ...
    return (
      <NotFoundPane
        ...
        onBack={() => router.push(buildMaterialsHref())}
      />
    );
  };
