...
export const listPurchases = async (): Promise<PurchasedItem[]> => {
    const response = await requestJsonAuth<PurchasedItemsListDto>('/api/shop/purchases');
    return response.items.map(toPurchasedItem);
};
